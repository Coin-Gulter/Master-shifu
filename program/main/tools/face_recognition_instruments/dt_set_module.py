...
# import array_additional_module
...
def delete_face_id_target_dt_set(face_target_path=FACE_JSON_TARGET_PATH, face_target_pass_path=FACE_JSON_TARGET_PASS_PATH):
    if os.path.exists(face_target_path):
        os.remove(face_target_path)
    if os.path.exists(face_target_pass_path):
        os.remove(face_target_pass_path)
...
def load_random_prepered_obj_from_recognize_dt_set(number=100, target_path=FACE_JSON_TARGET_PATH):
    ...
    for _ in range(number):
        ...
        random_target_id = array_additional_module.getFromDict(target_dict, random_target_folder)
...
def load_random_prepered_obj_from_password_dt_set(number=100, target_path=FACE_JSON_TARGET_PASS_PATH):
    ...
    for _ in range(number):
        ...
        target_pass = array_additional_module.getFromDict(target_dict, random_target_id)
