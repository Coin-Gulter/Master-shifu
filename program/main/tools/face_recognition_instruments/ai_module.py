...
FACE_TEST_DTSET = PATH_NOW + r'\program\data\face_id_data\test_face_dt'
...
class net_recognize_instrument(net_train):
    ...
    def train_iter(self, n_iter=1500, box=100, print_every=10, plot_every=10):
        ...
        for iter in range(1, n_iter+1):
            ...
            if (self.pairs == None) or (iter%box == 0):
                ...
                self.pairs = dt_set_module.load_random_prepered_obj_from_recognize_dt_set(number=box, target_path=self.face_dt_path)
    ...
    def test(self, number):
        ...
        for _ in range(number):
            ...
            folders = os.listdir(FACE_TEST_DTSET) 
            work_folder = random.choice(folders)
            if work_folder == 'client':
                who_is_on_img = 1
            else:
            ...
            random_folder_path = os.path.join(FACE_TEST_DTSET, work_folder)
            random_img_path = os.path.join(random_folder_path, random.choice(os.listdir(random_folder_path)))

            image = cv.imread(random_img_path, cv.IMREAD_COLOR)

            # img = cv.flip(image, 1)
            img, detection, detection_score = Fc_recog.faceDetector(image, False)
            ...
            if len(detection) != 0:
                ...
                dsize = (400, 400)
                image = cv.resize(img, dsize, interpolation = cv.INTER_AREA)
        ...
        print('percentage of test people = ', (number_of_recognizes/number*100))
    ...
    def use(self, input_img):
        ...
        return topi.item(), torch.exp(topv).item()
...
class net_pass_getting_instrument(net_train):
    ...
    def train_iter(self, n_iter=1500, box=100, print_every=100, plot_every=10):
        ...
        for iter in range(1, n_iter+1):
            ...
            if (self.pairs == None) or (iter%box == 0):
                print("Prepare new training object___________")
                self.pairs = dt_set_module.load_random_prepered_obj_from_password_dt_set(number=box, target_path=self.face_dt_path)
                print("__________New training object prepered")
    ...
    def use(self, input_list):
        ...
        while topi != 128 and inside_iter <= 20:
            ...
            topv_list.append(torch.exp(topv).item())
            rezult.append(chr(topi.item()))
            inside_iter += 1
...
class net_recognize(net_train):
    ...
    def train(self, password, train_iter_recognize=2000, validate_iter_recognize=1000, train_iter_pass_getter=1000, validate_iter_pass_getter=10, validate=True, test_iter_recognize=1000, test=True,
              path_face=FACE_DT_PATH, path_face_target=FACE_JSON_TARGET_PATH, path_pass_target=FACE_JSON_TARGET_PASS_PATH):

        dt_set_module.preper_face_id_target_dt_set(password, path=path_face, face_target_path=path_face_target, face_target_pass_path=path_pass_target)
        ...
        if validate:
            ...
            print('Validation ended_______________')
        if test:
            print('Testing.....................')
            self.net_recog_instrument.test(test_iter_recognize)
            print('Testing ended_______________')

        dt_set_module.delete_face_id_target_dt_set()

    def upload_weight(self):
        self.net_pass_getter_instrument.upload_weight()
        self.net_recog_instrument.upload_weight()

    def retrain(self, password, train_iter_recognize=2000, validate_iter_recognize=1000, train_iter_pass_getter=1000, validate_iter_pass_getter=10, validate=True, test_iter_recognize=1000, test=True,
              path_face=FACE_DT_PATH, path_face_target=FACE_JSON_TARGET_PATH, path_pass_target=FACE_JSON_TARGET_PASS_PATH):
        self.upload_weight()
        self.train(password, train_iter_recognize, validate_iter_recognize, train_iter_pass_getter, validate_iter_pass_getter, validate, test_iter_recognize, test,
                   path_face, path_face_target, path_pass_target)

    def use(self, inp_images):
        self.upload_weight()
        
        last_rez = None
        rezult = None
        recog_probality_list = []
        recog_probality_avg = 0

        for img in inp_images:
            topi, recog_probality_value = self.net_recog_instrument.use(img)

            recog_probality_list.append(recog_probality_value)
            
            if last_rez == None and last_rez == topi:
                last_rez = topi
            else:
                last_rez = None
                break
        
        recog_probality_avg = round(statistics.mean(recog_probality_list), 2)
        
        match last_rez:
            case 1:
                inp2pass_getter = self.make_zero_one_list(last_rez, 2)
                rezult, pass_probability_value = self.net_pass_getter_instrument.use(inp2pass_getter)
            case 0:
                rezult = False
            case _:
                rezult = None
        
        return rezult, recog_probality_avg
    

if __name__ == '__main__':
    ai_net_use = net_recognize()
    ai_net_use.train('jenneterrionQWE', train_iter_recognize=1500, train_iter_pass_getter=2000)
    ...
    #      img = cv.flip(img, 1)
    #      img, detection, detection_score = Fc_recog.faceDetector(img, False)
    #      img = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    #      if len(detection) != 0:
    #          face = detection[0][1]
    #          start_point_w = abs(face[0])
    ...
    #         # print(face)
    #          password, pass_probab, recog_probab = ai_net_use.use(img)
    #          print(f"Rezult = [{password}], password prob = [{pass_probab}], recog prob = [{recog_probab}]")
